...
export const batchesApi = {
  list: (params?: { status?: string; product?: string; customer?: string; search?: string; active?: boolean }) => {
  ...
  create: (data: { product: string; customer: string; plannedQuantity?: number; note?: string }) =>
    api.post<Batch>("/batches", data),
  update: (
    id: string,
    data: { plannedQuantity?: number; note?: string; status?: "dang_lam" | "chua_hoan_thanh"; active?: boolean }
