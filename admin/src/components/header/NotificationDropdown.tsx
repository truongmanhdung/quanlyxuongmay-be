...
export default function NotificationDropdown() {
  ...
  return (
    <div className="relative">
      ...
      >
        <span
          className={`absolute right-0 top-0.5 z-10 h-2 w-2 rounded-full bg-orange-400 ${!notifying || reports.length === 0 ? "hidden" : "flex"
            }`}
        >
      ...
      >
        <div className="flex items-center justify-between pb-3 mb-3 border-b border-gray-100 dark:border-gray-700">
          <h5 className="text-lg font-semibold text-gray-800 dark:text-gray-200">Duyệt sản lượng</h5>
          <button
        ...
        <ul className="flex flex-col h-auto overflow-y-auto custom-scrollbar">
          {reports.length === 0 && (
            <li className="py-8 text-center text-sm text-gray-400">Chưa có thông báo nào</li>
          )}
          {reports.map((r) => (
            <li key={r._id}>
              <DropdownItem
